...
uistack(h,'top'); % put red circle on top 
...
sinewave = ampl * sin( 2*pi * freq * time + phas ); % A*sin(2*pi*f*time + phase)
...
% generate the sine wave
csw = ampl * exp( 1i* (2*pi * freq * time + phas) ); % A*e^i*(2pi*f*t + phase)

figure(3), clf
subplot(211)
plot(time,real(csw), time,imag(csw),'linew',2) %two lines in 1 graph
xlabel('Time (sec.)'), ylabel('Amplitude')
...
srate = 1000;
time  = -1:1/srate:1; % length 2001

% signal create an artificial signal?
...
for fi=1:length(dps)
    ...
    size(sinew)
    % compute dot product
    dps(fi) = dot( sinew,signal ) / length(time); % average
end
...
mag = abs(dp_complex); %module
phs = angle(dp_complex); %the angle easily calculated
...
phases = linspace(0,7*pi/2,100); % this is different


% setup the plot: complex plane: cosine vs sine
figure(5), clf
subplot(223)
